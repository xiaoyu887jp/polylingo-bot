    ...
    for event in events:
        ...
        if event[\"type\"] == \"message\" and event[\"message\"][\"type\"] == \"text\":
            ...
            if user_text in [\"en\", \"ja\", \"zh-tw\", \"zh-cn\", \"th\", \"vi\", \"fr\", \"es\", \"de\", \"id\", \"hi\", \"it\", \"pt\", \"ru\", \"ar\", \"ko\"]:
                group_language_settings.setdefault(source_id, set()).add(user_text)
                reply_to_line(reply_token, [{\"type\": \"text\", \"text\": f\"✅ Languages set to {', '.join(group_language_settings[source_id])}\"}])
                continue

            langs = group_language_settings.get(source_id, set())
            messages = [{\"type\": \"text\", \"text\": f\"[{lang.upper()}] {translate(user_text, lang)}\"} for lang in langs]
